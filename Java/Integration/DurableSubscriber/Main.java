...
/**
 * DurableSubscriber Pattern - Enterprise Integration Pattern
 *
 * Category: Message Endpoints
 *
 * Intent:
 * Subscriber receives messages even when temporarily disconnected
 *
 * Key Concepts:
 ...
 *
 * When to Use:
 * - You need to implement subscriber receives messages even when temporarily disconnected
 ...
 * 1. Mobile apps: Handle offline periods
 * 2. Batch processing: Process accumulated messages
 * 3. Scheduled jobs: Receive messages while offline
 * 4. Maintenance windows: Catch up after downtime
 * 5. Reliable delivery: Never miss messages
 *
...
public class Main {
    ...
    private static final String PATTERN_NAME = "DurableSubscriber";
    ...
    public static void main(String[] args) {
        ...
        System.out.println("  Subscriber receives messages even when temporarily disconnected");
        System.out.println();

        // Initialize the pattern
        System.out.println("Initializing DurableSubscriber infrastructure...");
        DurableSubscriberImplementation implementation = new DurableSubscriberImplementation();
        System.out.println("  ✓ Infrastructure initialized");
        System.out.println();

        // Scenario 1: Mobile apps
        demonstrateScenario("Mobile apps", "Handle offline periods", implementation);

        // Scenario 2: Batch processing
        demonstrateScenario("Batch processing", "Process accumulated messages", implementation);

        // Scenario 3: Scheduled jobs
        demonstrateScenario("Scheduled jobs", "Receive messages while offline", implementation);

        // Scenario 4: Maintenance windows
        demonstrateScenario("Maintenance windows", "Catch up after downtime", implementation);

        // Scenario 5: Reliable delivery
        demonstrateScenario("Reliable delivery", "Never miss messages", implementation);
    ...
    private static void demonstrateScenario(
            ...
            DurableSubscriberImplementation implementation) {
