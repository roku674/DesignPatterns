...
/**
 * ClaimCheckPattern Pattern - Enterprise Integration Pattern
 *
 * Category: Message Transformation
 *
 * Intent:
 * Stores large message data and sends claim check for later retrieval
 *
 * Key Concepts:
 ...
 *
 * When to Use:
 * - You need to implement stores large message data and sends claim check for later retrieval
 ...
 * 1. Large file transfer: Store file, send reference
 * 2. Image processing: Store image, send ID
 * 3. Video processing: Store video, send token
 * 4. Document processing: Store document, send claim
 * 5. Binary data: Store blob, send reference
 *
...
public class Main {
    ...
    private static final String PATTERN_NAME = "ClaimCheckPattern";
    ...
    public static void main(String[] args) {
        ...
        System.out.println("  Stores large message data and sends claim check for later retrieval");
        System.out.println();

        // Initialize the pattern
        System.out.println("Initializing ClaimCheckPattern infrastructure...");
        ClaimCheckPatternImplementation implementation = new ClaimCheckPatternImplementation();
        System.out.println("  ✓ Infrastructure initialized");
        System.out.println();

        // Scenario 1: Large file transfer
        demonstrateScenario("Large file transfer", "Store file, send reference", implementation);

        // Scenario 2: Image processing
        demonstrateScenario("Image processing", "Store image, send ID", implementation);

        // Scenario 3: Video processing
        demonstrateScenario("Video processing", "Store video, send token", implementation);

        // Scenario 4: Document processing
        demonstrateScenario("Document processing", "Store document, send claim", implementation);

        // Scenario 5: Binary data
        demonstrateScenario("Binary data", "Store blob, send reference", implementation);
    ...
    private static void demonstrateScenario(
            ...
            ClaimCheckPatternImplementation implementation) {
